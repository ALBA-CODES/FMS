...
NodeMCMC::NodeMCMC(int MCL_, int PTL_, double temp_, SSP* prob_,Consumer* pool_, bool SP)
:MCL(MCL_)
,temp(temp_)
,prob(prob_)
,pool(pool_)
...
void NodeMCMC::runParallel(){
	...
	for (int i = 0; i < MCL; i++){
		...
		neigh.evalSol = prob->GPCAE(neighAux, &last_fullD, &pipes_countD, &last_seenD, &MD, &end);		
		setBestSol(neigh);
